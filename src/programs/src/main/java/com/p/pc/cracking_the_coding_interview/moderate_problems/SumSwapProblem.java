 ...
 *  each array (a, b) such that by swapping those elements the sum of both arrays become equal, i.e.
 *  sumA - a + b = sumB + a - b
 *  a - b = (sumA - sumB) / 2 => target
 *  RHS can be calculated easily and then for a given element in first array, we just need to find the corresponding element in second array
 *  such that a - b = target value.
 ...
 *  If diff between first elements of both arrays is equal to targetDiff then return, if diff < target then move to the next element in first
 *  array else move to the element in next array.
 */
public class SumSwapProblem {
    ...
    private int[] findPairToSwap(int[] array1, int[] array2) {
        ...
        Arrays.stream(array2).boxed().forEach(t -> array2Set.add(t));
