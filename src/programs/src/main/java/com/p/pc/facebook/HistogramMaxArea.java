 ...
 *
 * A brute approach will be to calculate the area at each bar of the histogram from the first bar but it will be O(n^2) solution.
 *
 * Approach:
 ...
 *  We can apply this approach as we know for sure that the left index value is smaller than the current index and bar the ith index is smaller
 *  than the popped bar and in between we would have had larger bars only and hence the popped bar height can be considered as smallest height.
 */
public class HistogramMaxArea {
    ...
    /**
     * Calculate the max area using stack where every new encountered bar having height more than the previous bar will be pushed to stack.
     * And for every new bar having smaller height, pop the top bar and calculate the area.
     * @param hist
     * @return
     */
    private static int calculateMaxArea(int[] hist) {
        ...
        while(i < hist.length) {
            if(stack.isEmpty() || hist[i] >= hist[stack.peek()]) {
                ...
                area = hist[topIndex] * (stack.peek() == null ? i : (i - 1) - stack.peek());
